...
/// <summary>
/// Инструмент "Прямоугольник" — рисование контура прямоугольника из блоков.
/// </summary>
public class RectangleTool : ITool
{
    ...
    public void OnRelease(Point gridPos, ProjectData data, Color activeColor)
    {
        ...
        {
            foreach (var pt in RenderUtils.GetRectangle(_startPos.Value, gridPos))
            {
    ...
    public void RenderPreview(Graphics g, Camera camera)
    {
        ...
        foreach (var pt in RenderUtils.GetRectangle(_startPos.Value, _currentPos))
        {
